...
def create_structural_decoders(f_maps, basic_module, conv_kernel_size, conv_padding,
    ...
    if half_decoder_layers:
        ...
        for i in range(0, len(f_maps) - 2, 2):
            in_channels = f_maps[i]
            out_channels = f_maps[i + 2]
...
class StructuralVariationalAutoEncoder(LightningModule):
    """
    Variational Autoencoder for learning structural embeddings from 3D voxel data.
    ...
    def __init__(
        ...
        # For a VAE, we produce mean and logvar from the encoder output
        self.to_mu = nn.Conv3d(f_maps[-1], latent_dim, kernel_size=2, stride=2)
    ...
    def decode(self, z):
        ...
        for decoder in self.decoders:
            x = decoder(None, x)  # No skip connections
    ...
    def get_embedding(self, x):
        """
        Get the mean (μ) of the latent distribution as the embedding.
        For many use cases, μ is a good representative embedding.
        """
        mu, logvar = self.encode(x)
        return mu  # or you could return reparameterize(mu, logvar) if you prefer
...
if __name__ == "__main__":
    ...
    model = StructuralVariationalAutoEncoder(
