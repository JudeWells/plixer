...
def generate_plots_from_results_df(df, output_dir, vis_deciles: bool = True, similarity_df: pd.DataFrame = None):
    """
    ...
    if similarity_df is not None:
        similarity_df['name'] = similarity_df.system_id
        df = df.merge(similarity_df, on='name', how='left')
        non_similar_df = df[
            (df['max_protein_similarity'] < 30) | 
            (df['max_ligand_similarity'] < 0.15) |
            (df['max_protein_similarity'].isna())
        ]
        print(f"N rows in non-similar df: {len(non_similar_df)}")
        print(f"N rows with seq-similarity < 30%: {len(df[df['max_protein_similarity'] < 30])}")
        print(f"N rows with lig-similarity < 0.3: {len(df[df['max_ligand_similarity'] < 0.3])}")
        print(f"N rows with no protein similarity: {len(df[df['max_protein_similarity'].isna()])}")
        dfs = [df, non_similar_df]
    else:
        dfs = [df]
    for df_ix, df in enumerate(dfs):
        prefix = "non_similar_" if df_ix == 1 else ""
